...
export interface AutomationsToolbarProps {
  ...
  onBulkAction: (action: 'run' | 'stop', automationIds: string[]) => Promise<void> | void
}
...
export function AutomationsToolbar({
  ...
  /**
   * Get filtered automations based on current filter state
   * Quest 4.4: Run/Stop All Filtered functionality
   */
   ...
   * Quest 4.4: Run/Stop All Filtered automations functionality
   */
  const handleBulkAction = async (action: 'run' | 'stop') => {
    const filteredAutomations = getFilteredAutomations()
    const selectedAutomationIds = filteredAutomations.map(automation => automation.id)

    if (selectedAutomationIds.length === 0) {
      toast({
        title: 'No automations found',
        description: 'No automations match the current filters.',
        variant: 'default'
      })
      return
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button
            ...
            aria-label="Run all filtered automations"
          >
          ...
          <Button
            ...
            disabled={isLoading}
            className="flex items-center gap-2"
            aria-label="Stop all filtered automations"
          >
