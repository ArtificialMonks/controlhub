...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // Record automation execution using our current method
      const success = payload.final_status === 'success'
      await automationRepository.recordAutomationRun(
        automationId, 
        payload.execution_time_ms || 0, 
        success
      )

      // Create a simple run record for logging
      const automationRun = {
        id: payload.execution_id || `run_${Date.now()}`,
        automation_id: automationId,
        status: payload.final_status
      }

      // TODO: Implement proper automation run tracking table
      // For now, we just update the stats via recordAutomationRun above
