...
import { formatDate, formatDateTime, formatRelativeTime } from '@/lib/core/utils/date-formatting'
...
// Import utility functions (keeping formatDuration and formatLastRun as they're utility functions)
import { formatDuration, formatLastRun } from '@/lib/data/mock-automations'
...
// Test data factories
const createTestAutomation = (overrides = {}) => ({
  ...
  success_rate: 95.5,
  n8n_run_webhook_url: 'https://test.example.com/webhook',
  n8n_stop_webhook_url: 'https://test.example.com/webhook/stop',
...
const testAutomations = [
  ...
  createTestAutomation({ id: 'test-automation-2', name: 'Test Automation 2', status: 'Stopped' })
]

const testClients = [
  createTestClient(),
  createTestClient({ id: 'test-client-2', name: 'Test Client 2' })
]
...
describe('Ecosystem Connectivity Integration', () => {
  ...
  it('should demonstrate functional integration', () => {
    ...
    // Test data integrity
    expect(testAutomations.length).toBeGreaterThan(0)
    expect(testClients.length).toBeGreaterThan(0)

    // Verify test data structure
    const firstAutomation = testAutomations[0]
